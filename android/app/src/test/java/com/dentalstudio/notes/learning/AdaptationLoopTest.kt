...
import com.dentalstudio.notes.data.generate.PromptBuilder
...
private class LearningStore {
    ...
    fun absorb(draft: String, final: String, templateId: String) {
        val existing = rules.toList()
        val mined = RuleMiner.mine(draft, final, templateId, existing)

        mined.rules.forEach { candidate ->
            val index = rules.indexOfFirst {
                it.type == candidate.type && it.scope == candidate.scope &&
                    it.pattern == candidate.pattern && it.replacement == candidate.replacement
            }
            if (index >= 0) {
                rules[index] = rules[index].copy(occurrences = rules[index].occurrences + 1)
            } else {
                rules += LearnedRule(
                    id = nextId++,
                    type = candidate.type,
        ...
        rules.replaceAll { rule ->
            if (rule.pattern in contradicted) rule.copy(contradictions = rule.contradictions + 1) else rule
        }
